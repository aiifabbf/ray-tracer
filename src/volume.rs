...
impl<T> ConstantMedium<T> {
    pub fn new(geometry: Arc<T>, density: f64) -> Self {
        Self {
            geometry: geometry,
            density: density,
        }
    ...
    T: Hit,
{
    ...
    fn hit(&self, ray: &Ray) -> Option<HitRecord> {
        if let Some(record1) = self.geometry.hit(&ray) {
            // 第一次hit
            let ray = Ray::new(
                record1.intersection().clone() + *ray.direction() * 1e-6,
                ray.direction().clone(),
            );
            if let Some(record2) = self.geometry.hit(&ray) {
                // 第二次hit
                let distanceInsideGeometry = record2.t(); // 光束在geometry内部飞行的距离
                let mut generator = thread_rng();
                let distance =
                    (-1.0 / self.density) * generator.gen_range(0.0 as f64, 1.0 as f64).log2(); // 为什么书这里要取log
